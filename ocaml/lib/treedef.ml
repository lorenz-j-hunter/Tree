          ...
          | (false, false) -> 
            ...
            for ch = 1 to dh do
              if blw = -1 then begin
                ...
                if (!cur_node)#get_stsize () <= trav_index then (); (*exit point.*)
                cur_node := List.nth ((!cur_node)#get_subtrees ()) trav_index;
                ref blw := 0;
            ...
            if dh = 1 then
              let trav_index = (d_abs_ind - 1) mod bf in
                cur_node := List.nth ((!cur_node)#get_subtrees ()) trav_index;
                let (a, b) = (!cur_node)#get_pair in Pair (a, b)
            else
              let (a, b) = (!cur_node)#get_pair in Pair (a, b)
          | (true, true) -> begin
            match self#get_root () with
